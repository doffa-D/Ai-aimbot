...
class FrameParser:
    ...
    def _convert_sv_to_tensor(self, frame):
        ...
        # Address UserWarning for tensor creation from list of numpy.ndarrays
        class_id_data = frame.class_id
        if isinstance(class_id_data, list):
            # If it's a list (potentially of ndarrays), convert to a single ndarray first
            class_ids_np = np.array(class_id_data, dtype=np.float32)
        elif isinstance(class_id_data, np.ndarray):
            class_ids_np = class_id_data.astype(np.float32)
        else:
            # Fallback for other types, though sv.Detections.class_id is typically np.ndarray
            class_ids_np = np.array([class_id_data], dtype=np.float32) 

        classes_tensor = torch.from_numpy(class_ids_np).to(self.arch)
